...
var Wireless = function() {
    ...
    self.configuration = {
        ...
        commands: {
            ...
            connect_wep: 'sudo iwconfig :INTERFACE essid ":ESSID" key s::PASSWORD',
            connect_wpa: 'sudo wpa_passphrase :ESSID :PASSWORD > wpa-temp.conf && sudo wpa_supplicant -Dwext -i:INTERFACE -cwpa-temp.conf',
    ...
    self.enable = function(callback) {
        self.emit('command_debug', false, self.configuration.translated.enable);
        exec(self.configuration.translated.enable, function(err, stdout, stderr) {
            if (err) {
                ...
                // Error: Command failed: wlan0: ERROR while getting interface flags: No such device
                // ^^ display prettier error
                throw err;
            }
    ...
    self.disable = function(callback) {
        self.emit('command_debug', false, self.configuration.translated.disable);
    ...
    self.leave = function(callback) {
        self.emit('command_debug', false, self.configuration.translated.leave);
        exec(self.configuration.translated.leave, function(err, stdout, stderr) {
            if (err) {
                console.log("There was an error when we tried to disconnect from the network");
                throw err;
            }
            if (callback) {
                callback();
    ...
    self._executeScan = function() {
        ...
        self.emit('command_debug', false, self.configuration.translated.scan);
    ...
    self._executeTrackConnection = function() {
        self.emit('command_debug', false, self.configuration.translated.stat);
        exec(self.configuration.translated.stat, function(err, stdout, stderr) {
            ...
            if (!foundOutWereConnected && self.connected) {
                self.connected = false;
                self.emit('leave', false, {ssid: 'WHO KNOWS'});
    ...
    self._executeConnectWEP = function(essid, password, callback_success, callback_failure) {
        ...
        self.emit('command_debug', false, command);
