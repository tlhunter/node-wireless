...
var Wireless = function() {
    ...
    self._executeConnectWEP = function(essid, password, callback_success, callback_failure) {
        exec('iwconfig :INTERFACE essid ":ESSID" key s::PASSWORD', function(err, stdout, stderr) {
            // todo
        });
    };
    ...
    self._executeConnectWPA = function(essid, password, callback_success, callback_failure) {
        exec('wpa_passphrase :ESSID :PASSWORD > wpa-temp.conf && wpa_supplicant -Dwext -i:INTERFACE -cwpa-temp.conf', function(err, stdout, stderr) {
            // todo
        });
    };
    ...
    self._executeConnectOPEN = function(essid, callback_success, callback_failure) {
        exec('iwconfig :INTERFACE essid :ESSID', function(err, stdout, stderr) {
            // todo
        });
    };
    ...
    self._decay = function() {
        ...
        for (address in self.networks) {
            ...
            if (this_network.last_tick == self.configuration.disappearThreshold+1) {
                self.emit('disappear', false, this_network);
            }
        }
    };
};
